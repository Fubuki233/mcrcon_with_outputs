...
DWORD WINAPI log_monitor_thread(LPVOID lpParam) {
    ...
    while (1) {
        char buffer[DATA_BUFFSIZE];
        // 检查是否有新的内容
        if (fgets(buffer, sizeof(buffer), file) != NULL) {
            printf("%s", buffer); // 输出新内容
        } else {
            Sleep(100); // 暂停一段时间，避免CPU占用过高
        }
    }
...
int main(int argc, char *argv[])
{
	...
	HANDLE log_thread = CreateThread(NULL, 0, log_monitor_thread, NULL, 0, NULL);
    if (log_thread == NULL) {
        fprintf(stderr, "Error: Could not create log monitor thread.\n");
        return 1;
    }
	int terminal_mode = 0;
